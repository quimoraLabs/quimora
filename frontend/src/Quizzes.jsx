...
import { enterFullScreen } from "./pages/dashboards/student/components/enterFullScreen";
...
function StudentQuiz() {
  ...
  // Function to handle the fullscreen flow before starting the quiz
  const handleStartExamFlow = useCallback(() => {
    try {
      // Execute global fullscreen utility
      enterFullScreen();
    } catch (error) {
  ...
  const handleStart = useCallback(
    (quiz) => {
      ...
      handleStartExamFlow(); // Trigger fullscreen before navigating
      // Navigate to quiz rules page, passing necessary quiz details
      navigate("/student/quiz/rules", {
        state: {
          quizId: quiz.id,
          title: quiz.title,
    ...
    [navigate, handleStartExamFlow],
  ); // Added handleStartExamFlow to dependencies

  // Define the headers for the DataTable
  const headers = useMemo(
    () => ["Quiz Title", "Time Limit", "Questions", ""],
    [],
  );
  ...
  const renderQuizRow = useCallback(
    (quiz) => (
      <>
        ...
        <td className="px-6 py-4 whitespace-nowrap text-sm text-muted">
          {quiz?.questionsCount} {/* Corrected to use questionsCount */}
        </td>
        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
          <button
            onClick={() => handleStart(quiz)}
            className="inline-flex items-center gap-2 px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-brand-mid hover:bg-brand-primary transition-all"
          >
